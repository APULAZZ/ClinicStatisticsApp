...
namespace ClinicStatisticsApp.UI
{
    public partial class ReviewReportWindow : Window
    {
        ...
        {
            if (sender is ReviewEntryViewModel item && e.PropertyName == nameof(ReviewEntryViewModel.EmployeeId))
            {
        ...
        private void UpdateEmployeeName(ReviewEntryViewModel item)
        {
        ...
        private void CopyFromPreviousButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var result = MessageBox.Show(
                    "Скопировать сотрудников из предыдущего месяца? Текущий список сотрудников в блоке ОТЗЫВЫ будет заменен.",
                    "Подтверждение",
        ...
        private void ReviewDataGrid_CellEditEnding(object sender, DataGridCellEditEndingEventArgs e)
        {
            ...
            {
                if (e.Row.Item is ReviewEntryViewModel item)
                {
                    UpdateEmployeeName(item);
                }
        ...
        {
            SmsTotalTextBlock.Text = _items.Sum(x => x.SmsSentCount).ToString();
            ReviewsTotalTextBlock.Text = _items.Sum(x => x.ReviewsLeftCount).ToString();
        }
    }
}