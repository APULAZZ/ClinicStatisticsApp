...
public sealed class MangoApiClient(HttpClient httpClient, MangoApiOptions options) : IMangoApiClient
{
    ...
    {
        using var document = JsonDocument.Parse(await SendAsync("/vpbx/cc/call/", new { entry_id = entryId }, cancellationToken));
    ...
    private async Task<string> PollAsync(string key, CancellationToken token)
    {
        ...
        for (var i = 0; i < 30; i++)
        {
            if (i > 0) await Task.Delay(TimeSpan.FromSeconds(2.5), token);
    ...
    private static string? FindTagId(JsonElement node) { if (node.ValueKind == JsonValueKind.Object) { foreach (var property in node.EnumerateObject()) { if (property.Name is "tag_id" or "id" && property.Value.ValueKind is JsonValueKind.String or JsonValueKind.Number) return property.Value.ToString(); var nested = FindTagId(property.Value); if (nested != null) return nested; } } else if (node.ValueKind == JsonValueKind.Array) foreach (var item in node.EnumerateArray()) { var nested = FindTagId(item); if (nested != null) return nested; } return null; }
}
